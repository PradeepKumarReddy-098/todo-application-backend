  ...
  switch (true) {
    ...
    case hasStatusProperty(request.query):
      if (status === "TO DO" || status === "IN PROGRESS" || status === "DONE") {
        ...
        response.send(data.map((eachItem) => camelCaseResult(eachItem)));
      } else {
        response.status(400);
...
app.get("/todos/:todoId/", async (request, response) => {
  ...
  const responseResult = await db.get(`select * from todo where id=${todoId};`);
  response.send(camelCaseResult(responseResult));
});
...
app.get("/agenda/", async (request, response) => {
  ...
  if (isMatch(date, "yyyy-MM-dd")) {
    ...
    const responseResult = await db.all(
      `select * from todo where due_date='${newDate}';`
    );
    response.send(responseResult.map((eachItem) => camelCaseResult(eachItem)));
  } else {
    response.status(400);
